...
namespace ScatterGL
{
	...
	void Model::loadModel(std::string pathName)
	{
	...
	std::vector<Texture> Model::loadMaterialTexture(aiMaterial* material, aiTextureType type, std::string typeName)
	{
		...
		for (unsigned int i = 0; i < material->GetTextureCount(type); i++)
		{
			...
			{
				Texture texture;
				texture.id = textureFromFile(str.C_Str(), directory, somebool);
	...
	unsigned int Model::textureFromFile(const char* pathName, const std::string& directory, bool gamma)
	{
		//std::string(pathName);
		std::string fileName;
		std::cout << pathName << '\n';
		...
		{
			std::cout << "Texture failed to load at path: " << pathName << '\n';
			stbi_image_free(data);
		}
		return textureID;
	}
